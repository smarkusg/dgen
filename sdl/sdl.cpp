...
#else
	SDL_WM_SetCaption(AMIGA_VERSION, "DGEN-SDL1");
#endif
	// Hide the cursor.
	SDL_ShowCursor(0);
	// Initialize screen.
...
#else
	DBUG ("[dgen] video: %dx%d, %u bpp (%u Bpp), %uHz\n",
#endif
...
#else
	DBUG(stderr, "[dgen] sound: %uHz, %d samples, buffer: %u bytes\n",
#endif
...
#else
		DBUG("[dgen] sdl: couldn't allocate sound buffers.\n");
#endif
		goto snd_error;
	}
