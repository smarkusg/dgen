...
fail:
	...
	fprintf(stderr, "Couldn't load battery RAM from `%s'\n",
		megad.romname);
#else
	DebugPrintF("[dgen] Couldn't load battery RAM from `%s'\n",
		megad.romname);
	...
	if (!dgen_region) {
		...
		if ((hz != dgen_hz) || (pal != dgen_pal) ||
			...
			printf("main: reconfiguring for region \"%c\": "
			       "%dHz (%s)\n", c, hz, (pal ? "PAL" : "NTSC"));
#else
			DebugPrintF("[dgen] main: reconfiguring for region \"%c\": "
			       "%dHz (%s)\n", c, hz, (pal ? "PAL" : "NTSC"));
...
#else
	DBUG("[dgen] %lu frames per second (average %lu, optimal %ld)\n",
	       fps, (frames / fpsclk), (long)dgen_hz);
#endif
